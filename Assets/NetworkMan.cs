...
public class NetworkMan : MonoBehaviour
{
    ...
    [Serializable]
    public class PositionUpdateMsg
    {
        //public commands cmd;
        public string message = "updateposition";
        ...
        try
        {
            switch (latestMessage.cmd) {
                case commands.NEW_CLIENT:
                    //Debug.Log("HEREHERE: " + newGameState.player.id.ToString());
...
// schedule the next receive operation once reading is done:
            ...
            if (newGameState != null && newGameState.player != null)
            {
                ...
                if (isTheSame == false)
                {
                    if (allPlayers.Contains(newplayer) == false)
                    {
                        ...
                        newplayer.myPosition.X = 0.0f;
                        newplayer.myPosition.Y = cubePosition + cubeList.Count;
                        newplayer.myPosition.Z = 6.0f;
                        ...
                        script = cloneTag.GetComponent<CubeScript>();
                        script.cubeid = newplayer.id;
                    }
                    myCubeid = allPlayers[allPlayers.Count-allPlayers.Count].id.ToString();
                }
            }
        }
    ...
    void UpdatePlayers(){
        Debug.Log("My Name Here: " + myCubeid);
    ...
    void UpdatePosition()
    {
        PositionUpdateMsg message = new PositionUpdateMsg();
        //message.cmd = commands.UPDATE_POSITION;
        //GameObject cubeX = GameObject.Find(myCubeid);
        message.position.X = GameObject.Find(myCubeid).transform.position.x;
        message.position.Y = GameObject.Find(myCubeid).transform.position.y;
        message.position.Z = GameObject.Find(myCubeid).transform.position.z;
